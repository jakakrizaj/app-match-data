...
class MatchingApp:
    ...
    def load_files(self):
        try:
            ...
            elif not pd.Series(["X", "x"]).isin(self.data1.columns).all():
            ...
            elif not pd.Series(["X", "x"]).isin(self.data2.columns).all():
                raise IndexError("Ne najdem podatkov o koordinatah za transformacijske podatke!")
    ...
    def create_tab3(self):
        ...
        create_dict_button = ttk.Button(self.tab3, text="Ustvari zbirko povezav", 
            command=lambda: self.create_matching_dict(self.data1, self.modified_data2, float(dbh_weight_entry.get())))
    ...
    def create_matching_dict(self, df_ref, df_las, dbh_weight=1):
        ...
        for index1, row1 in df_ref.iterrows():
            ...
            # Sort the list of dicts by distances, 10 closest trees
            newlist = sorted(mylist, key=lambda d: d["dist"])[:10]
        ...
        if self.dict_matches is not None:
            ...
            for key, list_dicts in self.dict_matches.items():
                ...
                if len(list_dicts) == 0:
                    ...
                    if second["dist"] - closest["dist"] > confidence:
